...
.DESCRIPTION
    Refactoré de 1625 lignes monolithiques en orchestrateur sans définition
    de fonction inline (BEA-283). Chaque phase est déléguée à un module
    step3/ spécialisé. Résultat structuré par phase → JSON lu par
    worker-step3.ps1.
    Modes de rejeu : -ForceNetworkConfigOnly (Network-only),
    -SkipInstantRecoveryStart (Incident recovery),
    -SkipNetworkAndPostConfig (IR-only),
    -Phases IRCommit,Network,HA (futur).
...
.PARAMETER Phases
    ...
    Valeurs : IRStart, IRCommit, Network, IntegrationServices, OS, HA,
    LiveMigration, BackupTag.
.EXAMPLE
...
.NOTES
    Part of vmware2hyperv — BEA-283 / BEA-261.7. PowerShell 7+.
#>
...
# 2. Modules step3/ — dot-source tous les .ps1 du répertoire step3/
$step3Dir = "$PSScriptRoot\step3"
if (Test-Path $step3Dir) {
    Get-ChildItem -Path $step3Dir -Filter '*.ps1' | ForEach-Object {
        . $_.FullName
    }
}

# 3. Initialisation des fonctions de session SCVMM
if (Get-Command Initialize-ScvmmSessionFunction -ErrorAction SilentlyContinue) {
...
# ═══════════════════════════════════════════════════════════════════════════
# Phase 2 : Instant Recovery — Start + Wait (Veeam)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# Phase 4 : Configuration réseau (Network Mapping)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# Phases 5-9 : Post-configuration (non-bloquantes)
# ═══════════════════════════════════════════════════════════════════════════
